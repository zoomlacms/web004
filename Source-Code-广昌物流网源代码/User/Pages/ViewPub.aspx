<%@ page language="C#" autoeventwireup="true" inherits="User_Pages_ViewPub, App_Web_orbb5nmy" viewStateEncryptionMode="Never" %>
...
<body>
    ...
    <a href="javascript:void();" onclick="javascript: window.open('ViewSmallPub.aspx?Pubid=<%#this.HiddenPubid.Value%>&ID=<%#Eval("ID") %>', 'newwindow', 'height=500, width=800, top=200, left=150, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no'); " >相关信息与回复管理</a>
    
    &nbsp;
        <a  href="javascript:void();" onclick="javascript: window.open('AddPub.aspx?Pubid=<%#this.HiddenPubid.Value%>&Parentid=<%#Eval("ID") %>', 'newwindow', 'height=500, width=800, top=200, left=150, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no');" >回复</a>&nbsp;
          <a href="PubView.aspx?Pubid=<%#this.HiddenPubid.Value%>&ID=<%#Eval("ID") %>" >查看</a>&nbsp;
       ...
       &nbsp;<a href="EditPub.aspx?Pubid=<%#this.HiddenPubid.Value%>&ID=<%#Eval("ID") %>" >修改</a>
        &nbsp;<a href=" Delpub.aspx?Pubid=<%#this.HiddenPubid.Value%>&ID=<%# Eval("ID")%>" OnClick="return confirm('不可恢复性删除数据,你确定将该数据删除吗？');">删除</a></td>
      </tr>
        </ItemTemplate>
        </asp:Repeater>
        <tr class="tdbg">
