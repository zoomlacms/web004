<%@ page language="C#" autoeventwireup="true" inherits="User_BidManage_Bidpaul, App_Web_rjsc3umk" viewStateEncryptionMode="Never" %>
<!DOCTYPE HTML>
<html>
<head runat="server">
<title>保价购买</title>
...
<div class="us_seta"  style="margin-top:5px;"><H1 id="tipss" runat="server" align="center">保价购买</H1></div>

<div style="width:100%; border:1px #CCC solid" id="nocont" runat="server">您还没有保价购买的商品! </div>
    ...
    <ItemTemplate>
        <dl class="Auction_HOR">
	        ...
	        <dd class="detail" style="text-align:left">
		        ...
		        <span>保价：<font color="#369A28">&yen;<%#GetName(Eval("shopID","{0}"),"3") %></font>&nbsp;&nbsp;市价：<span class="worth">&yen;<%#GetName(Eval("shopID","{0}"),"2") %></span></span>&nbsp;&nbsp; 成交价：<font color="#0093ED">&yen;<%#DataBinder.Eval(Container.DataItem, "InitPrice", "{0:F2}")%></font>
                <div align="left">有效期：<%#Eval("BegionTime")%> - <%#Eval("EndTime") %></div>
                </dd>
	        <dd class="bidder">
                <span id='bid_btn_<%#Eval("id") %>' class="button button-hilite"><span class="first-child">
                    <a class="userName" href="javascript:void(0)"  id="bidder_<%#Eval("id") %>" style="width:200px"><%#GetComTips(Eval("ID", "{0}"), Eval("DepVal", "{0}"))%></a>
